...
func Register_test(ranUIpAddr, upfIpAddr string, gnb *gnodeb.GNodeB) {

	var n int
	var sendMsg []byte
	var recvMsg = make([]byte, 2048)

	// TODO Sleep is currently added as the NG Setup procedure is currently carried
	// asynchronously. It needs to be synchronous
	time.Sleep(1 * time.Second)

	//TODO Temporarily receiving Default Amf socket connection. Should be event based
	amfConn := gnb.GetDefaultAmf().Conn
	if amfConn == nil {
		fmt.Println("Amf Connection not initialized")
		return
	}
